...
Model Set up for a model Envelope with Four detached shells from thermal pulse (shell 4 from optical) + continous Outflow with to account for pre&post thermal pulse emission

Total shell Mass devided Evenly between the four shells 

"""

def FourShells_plusCont_EvenMass(dust_file_name, fnu, nu, stellar_luminoisity, stellar_temperature,  stellar_radius, total_shell_mass, uant_distance, present_day_mlr, expansion_velocity,  continuous_envelope_inner_rad, continuous_envelope_outer_rad, Photon_Number, CPU_Number):
	...
	envelope_shell1.mass = total_shell_mass * 0.25              # 25% of total dust in shell 1
	...
	envelope_shell2.mass = total_shell_mass * 0.25              # 25% of total dust in shell 2
	...
	envelope_shell3.mass = total_shell_mass * 0.25        # 25% of total dust in shell 3 - Gas dominated
	...
	envelope_shell4.mass = total_shell_mass * 0.25              # 25% of total dust in shell 4
	...
	# Write out file
	Input_FourShells_ContEm_EvenMass = 'Model_FourShells+ContEm_EvenMass.rtin' #File which saves all the input above so that fortran can run internally for RT modelling
	Output_FourShells_ContEm_EvenMass = 'Model_FourShells+ContEm_EvenMass.rtout'
	
	model.write(Input_FourShells_ContEm_EvenMass)
	model.run(Output_FourShells_ContEm_EvenMass, mpi=True, n_processes=CPU_Number)

	return Input_FourShells_ContEm_EvenMass, Output_FourShells_ContEm_EvenMass
