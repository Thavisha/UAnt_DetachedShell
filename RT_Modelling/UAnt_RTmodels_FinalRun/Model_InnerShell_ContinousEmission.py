...
Model Set up for a model Envelope with Innermost detached shell from thermal pulse (shell 1 from optical) + continous Outflow with to account for pre&post thermal pulse emission 

"""

def InnerShell_plusCont(dust_file_name, fnu, nu, stellar_luminoisity, stellar_temperature,  stellar_radius, total_shell_mass, uant_distance, present_day_mlr, expansion_velocity,  continuous_envelope_inner_rad, continuous_envelope_outer_rad, Photon_Number, CPU_Number):
	...
	envelope_shell.rmin = (23.5 * uant_distance) * au          # Inner radius converted to au - from GD+2001&2003
	envelope_shell.rmax = (26.5 * uant_distance) * au          # Outer radius converted to au - from GD+2001&2003
	...
	# Write out file
	Input_InnerShell_ContEm = 'Model_InnerShell_ContEm.rtin' #File which saves all the input above so that fortran can run internally for RT modelling
	Output_InnerShell_ContEm = 'Model_InnerShell_ContEm.rtout'
	
	model.write(Input_InnerShell_ContEm)
	model.run(Output_InnerShell_ContEm, mpi=True, n_processes=CPU_Number)

	return Input_InnerShell_ContEm, Output_InnerShell_ContEm
