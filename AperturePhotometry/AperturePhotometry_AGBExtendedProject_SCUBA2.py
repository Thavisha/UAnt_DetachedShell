...
def aperture_photometry_data(star, input_filename, r_stellar, r_inner, r_outer, central_pixel_x, central_pixel_y, wavelength, data, data_unc):
	...
	#Callibration unc of 8% for SCUBA-2 data
	Cal_unc = 0.08 *  stellar_flux
...
if __name__=="__main__":
	...
	for source in source_list: 
		...
		print(aperture_photometry_output) #aperture_photometry_output.columns will give the astropy table column names

		#Creating output file with the headers needed 
		f = open(output_filename, 'w')
		f.write("Source" + ","  + "Center_X" + ","  + "Center_Y" + ","  + "Tot.Flux(mJy)" + ","  + "Tot.Unc(mJy) - Stellar+Cal" + "\n")
		f.close()
