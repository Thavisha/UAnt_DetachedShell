...
"""
################################################################################################################################
                    ################## Plotting resulting Temperature, Density and Beta Profiles for U Ant ########################
#################################################################################################################################
...
velocity = 2050000  #Terminal/Outflow velocity || units=cm/s || From DeBeck et al., 2010
radial_point_to_anchor_UMLM = 2 #Radial point at which to anchor Uniform Mass Loss Model (UMLM) on the density profile. Counting starts from 0 in python for the radial points. So 1st radial point = 0, 2nd radial point = 1, etc.... 
...
x_upper_limit = 25 #Plot x axis upto the X radial points. a little beyond where prior domination takes over. The rest is all prior dominated. 
...
ax1.errorbar(x_interp[nskip:x_upper_limit], Temperature[nskip:x_upper_limit], yerr=[Temperature_MinusUnc[nskip:x_upper_limit], Temperature_PlusUnc[nskip:x_upper_limit]], fmt='--o', color='crimson', capsize=2)
...
ax2.errorbar(x_interp[nskip:x_upper_limit], Density[nskip:x_upper_limit], yerr=[Density_MinusUnc[nskip:x_upper_limit], Density_PlusUnc[nskip:x_upper_limit]], fmt='--^', color='indigo', capsize=2)
ax2.plot(x_interp[nskip:x_upper_limit], np.log10(Uniform_MassLoss_Model_scaled[nskip:x_upper_limit]), '--', color='chocolate', lw=3) #Plotting the scaled UMLM in log form
...
ax3.errorbar(x_interp[nskip:x_upper_limit], Beta[nskip:x_upper_limit], yerr=[Beta_MinusUnc[nskip:x_upper_limit], Beta_PlusUnc[nskip:x_upper_limit]], fmt='--s', color='darkgreen', capsize=2)
...
#ax3.set_xlim([0,145])
beta_lower_limit = (min(Beta) - Beta_MinusUnc[np.argmin(Beta)]) - 0.5
...
ax4 = ax1.twiny()
ax4.errorbar(x_interp_time[nskip:x_upper_limit], Temperature[nskip:x_upper_limit], color='none', mec='none')
