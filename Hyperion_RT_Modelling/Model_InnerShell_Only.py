...
Model Set up for a model Envelope with Innermost detached shell from thermal pulse (shell 1 from optical). All the shell mass is put in shell 1 in this case. 

"""

def InnerShell_Only(dust_file_name, fnu, nu, stellar_luminoisity, stellar_temperature,  stellar_radius, total_shell_mass, uant_distance, expansion_velocity, MaxCSE_outer_rad, Photon_Number, CPU_Number):
	...
	# Write out file
	Input_InnerShell_Only = 'Model_InnerShell_Only.rtin' #File which saves all the input above so that fortran can run internally for RT modelling
	Output_InnerShell_Only = 'Model_InnerShell_Only.rtout'
	
	model.write(Input_InnerShell_Only)
	model.run(Output_InnerShell_Only, mpi=True, n_processes=CPU_Number)

	return Input_InnerShell_Only, Output_InnerShell_Only
