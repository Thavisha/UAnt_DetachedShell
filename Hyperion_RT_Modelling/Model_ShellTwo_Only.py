...
Model Set up for a model Envelope with 2nd detached shell (shell 2 from optical). All the shell mass is put in shell 2 in this case. 

"""

def ShellTwo_Only(dust_file_name, fnu, nu, stellar_luminoisity, stellar_temperature,  stellar_radius, total_shell_mass, uant_distance, expansion_velocity, MaxCSE_outer_rad, Photon_Number, CPU_Number):
	...
	envelope_shell.rmin = (34 * uant_distance) * au          # Shell 2 inner radius converted to au - from GD+2001&2003
	envelope_shell.rmax = (40 * uant_distance) * au          # Shell 2 outer radius converted to au - from GD+2001&2003
	...
	# Write out file
	Input_ShellTwo_Only = 'Model_ShellTwo_Only.rtin' #File which saves all the input above so that fortran can run internally for RT modelling
	Output_ShellTwo_Only = 'Model_ShellTwo_Only.rtout'
	
	model.write(Input_ShellTwo_Only)
	model.run(Output_ShellTwo_Only, mpi=True, n_processes=CPU_Number)

	return Input_ShellTwo_Only, Output_ShellTwo_Only
