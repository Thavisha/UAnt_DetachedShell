...
def FourShells_plusCont_UnevenMass(dust_file_name, fnu, nu, stellar_luminoisity, stellar_temperature, stellar_radius, total_shell_mass, uant_distance, present_day_mlr, expansion_velocity,  continuous_envelope_inner_rad, continuous_envelope_outer_rad, Photon_Number, CPU_Number):
	...
	# Write out file
	Input_FourShells_ContEm_UnevenMass = 'Model_FourShells_ContEm_UnevenMass.rtin' #File which saves all the input above so that fortran can run internally for RT modelling
	Output_FourShells_ContEm_UnevenMass = 'Model_FourShells_ContEm_UnevenMass.rtout'
	
	model.write(Input_FourShells_ContEm_UnevenMass)
	model.run(Output_FourShells_ContEm_UnevenMass, mpi=True, n_processes=CPU_Number)

	return Input_FourShells_ContEm_UnevenMass, Output_FourShells_ContEm_UnevenMass
