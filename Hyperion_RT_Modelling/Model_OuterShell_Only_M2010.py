...
Model Set up for a model Envelope with Outermost detached shell from thermal pulse (shell 4 from optical)

Shell Radii - Taken from Maercker et al., 2010 - Scattered light observations (shell 4 is the outer most)

"""

def OuterShell_Only_M2010(dust_file_name, fnu, nu, stellar_luminoisity, stellar_temperature,  stellar_radius, total_shell_mass, uant_distance, expansion_velocity, MaxCSE_outer_rad, Photon_Number, CPU_Number):
	...
	envelope_shell.rmin = (46.5 * uant_distance) * au          # Inner radius converted to au - from Maercker+2010 - Scattered light data
	envelope_shell.rmax = (53.5 * uant_distance) * au          # Outer radius converted to au - from Maercker+2010 - Scattered light data
	...
	# Write out file
	Input_OuterShell_Only_M2010 = 'Model_OuterShell_Only_M2010.rtin' #File which saves all the input above so that fortran can run internally for RT modelling
	Output_OuterShell_Only_M2010 = 'Model_OuterShell_Only_M2010.rtout'
	
	model.write(Input_OuterShell_Only_M2010)
	model.run(Output_OuterShell_Only_M2010, mpi=True, n_processes=CPU_Number)

	return Input_OuterShell_Only_M2010, Output_OuterShell_Only_M2010
