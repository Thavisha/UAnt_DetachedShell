...
Shell Radii - Taken from Kerschbaum et al., 2010 - PACS Observations. (Technically it's shell 3 that is outer most here) 

"""

def OuterShell_Only_K2010(dust_file_name, fnu, nu, stellar_luminoisity, stellar_temperature,  stellar_radius, total_shell_mass, uant_distance, expansion_velocity, MaxCSE_outer_rad, Photon_Number, CPU_Number):
	...
	envelope_shell.rmin = (34 * uant_distance) * au          # Inner radius converted to au - from Kerschbaum+2010 - PACS data
	envelope_shell.rmax = (46 * uant_distance) * au          # Outer radius converted to au - from Kerschbaum+2010 - PACS data
	...
	# Write out file
	Input_OuterShell_Only_K2010 = 'Model_OuterShell_Only_K2010.rtin' #File which saves all the input above so that fortran can run internally for RT modelling
	Output_OuterShell_Only_K2010 = 'Model_OuterShell_Only_K2010.rtout'
	
	model.write(Input_OuterShell_Only_K2010)
	model.run(Output_OuterShell_Only_K2010, mpi=True, n_processes=CPU_Number)

	return Input_OuterShell_Only_K2010, Output_OuterShell_Only_K2010
