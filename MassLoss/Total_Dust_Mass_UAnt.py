...
if __name__=="__main__":
	...
	calculated_densitites_output_filename = 'UAnt_TotalMass_output.csv'
	#print source_list


	f = open(calculated_densitites_output_filename, 'w')
	f.write("source" + "," + wavelength+"_three_sigma(arcsec)" + "," + wavelength+"_three_sigma(cm)" + "," + wavelength+"_three_sigma(pc)" + "," + "DeBeck_etal_2010_Terminal_Velocity_fromCOdata(cm/s)" + "," + "3sigma_radius_850_Time(years)(Time=Dist/Vel)unit_sToYr=*3.171e-8" + "," + "Intergrated_Dust_Mass_fromDensityProfile(Msun)" + "\n")
	f.close()
	...
	for source in input_data: 
		...
		outstring = str(star)  + ","  + str(three_sigma_arcsec)  + ","  + str(three_sigma_cm)   + ","  + str(three_sigma_pc) + "," + str(DeBeck_Terminal_Velocity) + "," + str(three_sigma_radius_inTimeUnits) + "," + str(Integrated_Density) + "\n"
		f.write(outstring)	
		f.close()
