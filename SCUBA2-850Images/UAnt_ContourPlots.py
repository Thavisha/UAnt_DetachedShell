...
cmap_type_dict =  {'850':'gnuplot2', '450':'gnuplot2'}#'magma'}
...
vmins={'850':0.0075, '450':0.1}
vmaxs={'850':0.15, '450':0.75}
vmids={'850':.025, '450':0.2}
fig_size_dict = {'850': 90/3600., '450': 70/3600.}
fig_size = fig_size_dict[str(wavelength)]

Source_Data = Table.read('UAnt_ContourPlot_Info.csv', format='ascii.csv')
...
for Source in Source_Data: 
	...
	vmax_val = Source['Vmax_use'] #Vmax value estimated by checking the value of the brightest pixel and then reducing a small value from it to allow for saturation. #0.171029
	...
	#fig.show_colorscale(vmin=0.001, vmax=0.25, cmap=cmap_type, stretch=stretch_type) #vmin=0.0001 for log scale - vmin can not be zero for log scale so we chose an arbitrary number| vmax=vmax_val can be set if we want to.
	fig.show_colorscale(vmin=vmins[str(wavelength)], vmax=vmaxs[str(wavelength)], vmid=vmids[str(wavelength)], cmap=cmap_type, stretch=stretch_type)
	...
	fig.show_circles(x_world, y_world, shell1, edgecolor='white', dashes='--', linewidth=2) #3sig.Radius (xcenter, ycenter, radius) 44"for CIt6
	fig.show_circles(x_world, y_world, shell2, edgecolor='white', dashes=':', linewidth=2) #3sig.Radius (xcenter, ycenter, radius) 44"for CIt6
	...
	fig.save(star + '_' + 'ContourPlot' + '_' + str(wavelength) + '_' + str(cmap_type) + '_' + str(stretch_type) + '_vMinMaxAdjusted_WithShells.png', dpi=300)
	plt.show()
	exit()
